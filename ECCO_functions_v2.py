...
def Fast_v4(nc_path, lake_file, outputprefix,lstart=0,lstop=275265,
                       ...
                       rprt_loop=False):
    '''
    Created on 4-May-2015 to use new HDF and csv metadata.
    Input:
    ...
    clim_dat,rlat,rlon,timeCDX,metadata,txtfname = Read_CORDEX_V2(nc_path) # CORDEX Read
    ...
    orog = Height_CORDEX()                # EUR-11 surface height data 
    ...
    for n in dolakes:
        ...
        if lake_meta.npix[EB_id] == 1:         # ONE PIXEL LAKES <<<
            ...
            tlist = Weighted_Mean_3D(weight_mask, sub_clim, chatty=False) # Make timeseries
            
            if plots:
                Show_LakeAndData(lake_rprj,dat_loaded[0,:,:],rlat,rlon,zoom=3.)
                Preview_Weights(lake_rprj,weight_mask,sub_rlat,sub_rlon)
                plt.plot(tlist)
        ...
        Write_HDF(f,EB_id,tlist,offset,lake_meta.area[n])  # Write inside function
        feature1=0
        lake_feature = 0
...
def Pixel_Weights(lake_in, datin,lat_atts,lon_atts):
    '''
    This is the second version of Pixel_Weights(), crated 29/04/2015 - BAL
    it should replace all functions of pixel_weights and fix bugs. The old version
    is maintained below as Pixel_Weights_old().
    Four possibilities:
    ...
    This function takes a lake (or any polygon shape including catchment) and
    creates a temporay bounding box shape to represent a pixel (as a vector)
    then performs various tests to see if (and how many) x,y positions of the lake
    are within the pixel, or, if indeed the lake itself is within the pixel, or
    if the pixel is within the lake! (options 1 to 4). It will then take 
    appropriate action if the tests are passed to trim the shape and calculate a
    fractional area per grid cell (that is aware of islands).
    '''
    fillone = 0
    lout = lake_in
...
def Weighted_Mean_3D_old(weight_mask,all_time_clim,chatty):
    '''
    New changes in the non-old version above. This version is fine
    though as far as I can see. The newer version is just easier 
...
def Fast_v3_old(nc_path, lake_file, outputprefix,lstart=0,lstop=275265,
                       hexlist=None,tt=None,plots = False,rprt=False,sbar=False,
                       rprt_loop=False):
    '''
    This has been replaced by Fast_v4()
    Input:
...
def Pixel_Weights_old(lake_in, datin,lat_atts,lon_atts):
    '''
